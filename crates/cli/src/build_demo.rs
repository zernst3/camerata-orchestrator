...
pub async fn run_build_demo() -> anyhow::Result<()> {
    ...
    let impl_spawn = prepare_session(&impl_session_dir, &gateway_bin, &implementer_role)?;
    let impl_driver = impl_spawn.driver.with_worktree(&worktree);

    let tester_session_dir = root.join("tester-session");
    let tester_spawn = prepare_session(&tester_session_dir, &gateway_bin, &tester_role)?;
