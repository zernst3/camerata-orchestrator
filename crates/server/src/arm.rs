...
pub struct GateRule {
    ...
    #[serde(default)]
    pub option: Option<String>,
    /// For custom rules (`CUSTOM-*`) only: the directive body, so reconcile can round-trip the
    /// custom rule back into the project (corpus rules omit this — their source is the corpus).
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub body: Option<String>,
    /// For custom rules only: the repos the rule is scoped to, so reconcile restores the scoping.
...
pub fn arm_files_for_repo(
    ...
    for c in custom {
        gate.push(GateRule {
            ...
            option: None,
            // Carry the body + scoping so reconcile can rebuild the custom rule in the project.
