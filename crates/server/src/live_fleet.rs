...
pub async fn execute_live_run_tiered(
    ...
    // The compliance-safety gate (see `docs/design/2026-09-22_per-project-backend.md`): the
    // active project's backend setting, resolved by the caller (`state.projects.active()`)
    // and threaded into the `ServerOrchestratorDriverFactory` this run builds for its
    // lead/children.
