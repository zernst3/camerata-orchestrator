...
use camerata_checks::vcs_action::{
    AdoLinkConfig, BranchNamingConfig, CommitDocConfig, ConventionalCommitConfig, IdLocation,
    ProcessRuleConfig, StoryIdFormat,
};
use camerata_server::vcs_choke::{
    compliant_machine_commit_message, compliant_machine_pr, gated_branch, gated_commit,
    gated_commit_or_bypass, gated_pr, gated_pr_or_bypass, numeric_story_id, ChokeError,
};
...
// ════════════════════════════════════════════════════════════════════════════════════
// Refinement #1: Camerata's OWN machine commits are COMPLIANT (not bypassed)
// ════════════════════════════════════════════════════════════════════════════════════

/// An enterprise-realistic project: conventional-commit shape, a substantive body, AND a
/// story-id reference in the body. This is exactly `full_doc_config()`, restated here so the
/// intent (a project with real process rules the machine must satisfy) is explicit.
fn enterprise_config() -> ProcessRuleConfig {
    ProcessRuleConfig::default()
}

/// THE REFINEMENT #1 CLAIM: a commit message GENERATED by Camerata's run path (from the
/// story id) is itself COMPLIANT and PASSES the HARD-BLOCK gate — no bypass required. This is
/// the moat: machine commits go through `gated_commit`, not `gated_commit_or_bypass`.
#[test]
fn refinement1_generated_machine_commit_passes_hard_block() {
    let cfg = enterprise_config();
    let story_id = "acme/widgets#42";
    let numeric = numeric_story_id(story_id);
    assert_eq!(numeric, "42", "the numeric id is the tail after '#'");

    let msg = compliant_machine_commit_message(
        &cfg,
        "feat",
        &format!("implement story {story_id} on camerata/{numeric}"),
        &numeric,
    );

    gated_commit(&cfg, &msg).unwrap_or_else(|e| {
        panic!("the generated machine commit must PASS the hard-block gate, not need a bypass: {e}\nmessage was:\n{msg}")
    });
}

/// The inverse moat: if the run has no usable story id, the generated message cannot carry a
/// compliant reference, so `gated_commit` HARD-BLOCKS. A missing story id surfaces as a real
/// error (a bug to fix), never a silent bypass.
#[test]
fn refinement1_machine_commit_without_story_id_hard_blocks() {
    let cfg = enterprise_config();
    let msg = compliant_machine_commit_message(&cfg, "feat", "implement the story", "");
    let err = gated_commit(&cfg, &msg)
        .expect_err("a machine commit with no story id must hard-block under the doc rule");
    match err {
        ChokeError::Blocked(v) => assert!(
            v.iter().any(|x| x.rule_id == "PROCESS-COMMIT-DOC-1"),
            "the commit-doc rule must fire when the story id is missing: {v:?}"
        ),
        other => panic!("expected Blocked, got {other:?}"),
    }
}

/// The generator adapts to a project's custom story-id + ADO configuration (Azure Boards
/// `AB#<id>` in the subject). The generated message passes that project's exact gate.
#[test]
fn refinement1_generated_commit_passes_ado_subject_id_config() {
    ...
    let msg = compliant_machine_commit_message(&cfg, "fix", "resolve PR #7 feedback", "1234");
    gated_commit(&cfg, &msg)
        .unwrap_or_else(|e| panic!("generated commit must pass the ADO subject-id gate: {e}\n{msg}"));
}

// ════════════════════════════════════════════════════════════════════════════════════
// Refinement #2: every PR-open site is gated (UoW / Ship PR-open)
// ════════════════════════════════════════════════════════════════════════════════════

/// A machine PR GENERATED by the UoW / Ship PR-open path is compliant and passes the
/// HARD-BLOCK PR gate. This is the moat for the non-governance PR-open sites.
#[test]
fn refinement2_generated_machine_pr_passes_hard_block() {
    let cfg = enterprise_config();
    let (title, body) = compliant_machine_pr(
        &cfg,
        "Camerata: acme/widgets#42",
        "Opened by Camerata for story `acme/widgets#42`.",
        "42",
    );
    gated_pr(&cfg, &title, &body)
        .unwrap_or_else(|e| panic!("the generated machine PR must PASS the hard-block gate: {e}\n{title}\n{body}"));
}

/// A UoW PR-open with a VIOLATING title/body (empty body, no story id) is gated and blocked
/// at its chokepoint — the same enforcement the governance PR-opens already had.
#[test]
fn refinement2_uow_pr_open_with_violating_body_is_gated() {
    let cfg = enterprise_config();
    // Simulate what an ungated raw PR-open would have produced pre-refinement: a bare title and
    // empty body. The gate must block it.
    assert!(
        gated_pr(&cfg, "Camerata: some-story", "").is_err(),
        "a UoW PR-open with a violating (empty) body must be blocked by the gate"
    );
}

// ════════════════════════════════════════════════════════════════════════════════════
// Refinement #3: branch-creation gating (PROCESS-BRANCH-NAMING-1)
// ════════════════════════════════════════════════════════════════════════════════════

/// A branch-naming rule that is ACTIVE blocks a non-conforming name at the branch-create
/// chokepoint and allows a conforming one.
#[test]
fn refinement3_branch_gate_enforces_when_rule_active() {
    let mut cfg = ProcessRuleConfig::default();
    ...
    let err = gated_branch(&cfg, "camerata/acme-widgets-42")
        .expect_err("a non-conforming branch name must block when the rule is active");
    match err {
        ChokeError::Blocked(v) => assert!(
            v.iter().any(|x| x.rule_id == "PROCESS-BRANCH-NAMING-1"),
            "the branch-naming rule must fire: {v:?}"
        ),
        other => panic!("expected Blocked, got {other:?}"),
    }

    gated_branch(&cfg, "feature/export-endpoint")
        .expect("a conforming branch name must pass when the rule is active");
}

/// When branch-naming is INACTIVE (the opt-in default), any branch name is allowed. Wiring
/// the gate must not regress the default no-op behaviour.
#[test]
fn refinement3_branch_gate_is_noop_when_rule_inactive() {
    let cfg = ProcessRuleConfig::default();
    gated_branch(&cfg, "camerata/acme-widgets-42")
        .expect("any branch name passes when branch-naming is disabled (default)");
    gated_branch(&cfg, "literally/anything-goes")
        .expect("any branch name passes when branch-naming is disabled (default)");
}
