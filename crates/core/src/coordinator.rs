...
impl<'a> Coordinator<'a> {
    ...
    pub async fn run(&self, role: &Role, task: &str) -> Result<RunReport, CoordinatorError> {
        ...
        // 3. ONE bounce-and-revise pass: append the violated rule ids to the
        //    task so the agent knows exactly what to fix, then re-run + re-check.
...
pub fn build_revise_task(original: &str, violated: &[RuleId]) -> String {
    let ids = violated
        .iter()
        .map(|r| r.0.as_str())
        .collect::<Vec<_>>()
