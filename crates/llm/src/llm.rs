...
///
/// The const was RELOCATED to `camerata_api_types::project` (Phase A of the DTO
/// extraction; `camerata_app_core::project` re-exports it from there). Re-exported here
/// FROM API-TYPES DIRECTLY — not via app-core — so this crate stays below
...
///
/// The audit pipeline (`ai_audit` in `camerata-server`) used to take a concrete `&Llm`
/// everywhere.
...
/// `&dyn LlmPort` and a test can substitute a stub that always errors.
///
/// OBJECT-SAFE by construction (so the audit can pass `&dyn LlmPort` down through ~10
...
/// implementor (see `impl LlmPort for Llm`).
///
...
/// A completion result.
///
/// Relocated to `camerata_api_types::llm` (Phase A of the DTO extraction) — a pure
/// serde-only wire body with no dependency on this module's transport code. Re-exported
/// so every existing `crate::llm::LlmResponse` call site keeps resolving unchanged.
pub use camerata_api_types::llm::LlmResponse;
...
impl Llm {
    ...
    pub async fn submit_batch(&self, items: Vec<BatchItem>) -> anyhow::Result<BatchSubmitResult> {
        ...
        if items.len() > 100_000 {
            ...
            eprintln!(
                "[camerata-llm] batch has {} items, which exceeds the 100k cap — \
...
pub fn parse_batch_results_jsonl(jsonl: &str) -> anyhow::Result<Vec<BatchResultRow>> {
    ...
    for line in jsonl.lines() {
        ...
        let Ok(v) = serde_json::from_str::<serde_json::Value>(line) else {
            ...
            eprintln!("[camerata-llm] skipping malformed batch result line: {line}");
            continue;
        };
        let custom_id = match v["custom_id"].as_str() {
            ...
            None => {
                #[cfg(debug_assertions)]
                eprintln!("[camerata-llm] batch result line missing custom_id, skipping");
                continue;
            }
        };
...
//
// Object-safe: satisfies the same `LlmPort` trait as `Llm`, so callers never need to
// know which backend they hold.

/// A `LlmPort` that routes bare-LLM calls through OpenRouter's chat-completions API.
///
...
impl OpenRouterCompleter {
    ...
    ///
    /// `pub` since the extraction into `camerata-llm`: called by the `ApiAgentDriver`
    /// in `camerata-server`.
    ...
    /// Expose the resolved API key for callers that need to post directly with tool
    /// schemas (the `ApiAgentDriver`'s agentic HTTP call, in `camerata-server`).
    pub fn api_key_for_agent(&self) -> String {
        self.api_key.clone()
    }

    /// Expose the session id so the `ApiAgentDriver` (in `camerata-server`) can embed it
    ...
    async fn call_api_inner(
        ...
        if let Some(d) = or_cache_discount {
            if d > 0.0 {
                eprintln!(
                    "[camerata-llm] OpenRouter cache_discount={d:.2} for session={} model={model_returned}",
                    self.session_id
                );
            }
        }
...
#[async_trait::async_trait]
impl LlmPort for OpenRouterCompleter {
    async fn complete(&self, req: LlmRequest) -> anyhow::Result<LlmResponse> {
...
// return the right `Arc<dyn LlmPort>`. Callers that currently do `state.llm()` for a
...
/// Build the right `Arc<dyn LlmPort>` for `model_id` based on what the registry says
/// its provider is.
///
...
///
/// For each model: builds the right `LlmPort` via `build_completer`, then calls
...
mod tests {
    ...
    // `camerata_api_types::llm::tests` along with the struct + its impl.
