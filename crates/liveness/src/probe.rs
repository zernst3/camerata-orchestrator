//! Async liveness helpers: the mtime probe and the `HeartbeatFn` callback type.
//!
//! Two complementary signals feed a [`HeartbeatFn`] so a long-running tool (e.g.
//! `cargo clippy` cold-compiling a repo with native deps like rocksdb) keeps the
//! parent job alive even when it emits no stdout lines for several minutes:
//!
//! 1. **Output-line signal** — the existing `stream_subprocess` path already fires
//!    `on_activity()` on every stdout line. No new code here; documented for
//!    discoverability.
//!
//! 2. **Build-dir mtime probe** — [`spawn_mtime_probe`] starts a `tokio::spawn`
//!    background task that polls `newest_mtime(dir)` every ~15s. When the newest
//!    file mtime advances (i.e. `cargo` wrote something under `target/` even without
//!    emitting a line), it fires the supplied [`HeartbeatFn`]. This is the **rivet
//!    fix**: `cargo clippy` compiles rocksdb for 8+ minutes with no clippy output,
//!    but it continuously writes into `target/`, so the probe fires heartbeats
//!    throughout.
//!
...
/// Callback type: fired once per stdout line received from the agent subprocess, or
/// once per mtime advance from the build-dir probe. Callers that track run activity
/// update their `last_activity_ms` here. `Arc` so it is cheaply cloneable.
...
/// `camerata_agent::DEFAULT_AGENT_TOTAL_TIMEOUT_SECS` (1 hour) so the probe never
/// outlives the absolute maximum a tool run is allowed to take.
pub const MTIME_PROBE_MAX_DURATION: Duration = Duration::from_secs(3_600);
