...
/// per call or via `CAMERATA_LLM_MODEL`. Relocated here from the adapter's `llm` module
/// (D2) so the pure `project` domain can seed step models without depending on the
/// transport-layer LLM code; `llm` re-exports it (`pub use ...::project::DEFAULT_MODEL`).
