...
/// The `UowStage` enum itself + its pure inherent impl (`label`, `wire_str`,
/// `from_wire`, `ordinal`) were relocated to `camerata_api_types::lifecycle` (Phase A of
/// the DTO extraction) — a pure-serde leaf crate with no dependency on
/// `camerata_worktracker`. Re-exported so every existing `crate::lifecycle::UowStage` /
/// `camerata_app_core::lifecycle::UowStage` call site keeps resolving unchanged.
///
/// The stage-TRANSITION methods below (`begin_investigation`, `approve_decisions`,
/// `start_development`, `finish_development`, `sign_off`) stay in this module — they
/// import `camerata_worktracker::investigation::*`, a domain dependency the wire-
/// contract leaf crate must not carry. They are carried forward as the
/// [`UowTransitions`] extension trait, implemented here for the foreign `UowStage`
/// type (legal under Rust's orphan rules: the trait is local even though the type
/// isn't), so every existing `stage.begin_investigation()`-style call site keeps
/// compiling with method-call syntax unchanged.
pub use camerata_api_types::lifecycle::UowStage;

/// The pure lifecycle state-transition methods on [`UowStage`]. See the module-level
/// doc for why these live here (not on `UowStage` itself) as an extension trait.
pub trait UowTransitions: Sized {
    /// Intake → Investigating. Always allowed from `Intake`; rejected otherwise.
    fn begin_investigation(self) -> Result<UowStage, TransitionError>;
    ...
    fn approve_decisions(self, decisions: &[DecisionRecord]) -> Result<UowStage, TransitionError>;
    ...
    fn start_development(self, decisions: &[DecisionRecord]) -> Result<UowStage, TransitionError>;
    ...
    fn finish_development(self) -> Result<UowStage, TransitionError>;

    /// AwaitingQa → SignedOff. The architect's explicit, never-automatic gate.
    /// Allowed only from `AwaitingQa`.
    fn sign_off(self) -> Result<UowStage, TransitionError>;
}

impl UowTransitions for UowStage {
    fn begin_investigation(self) -> Result<UowStage, TransitionError> {
        match self {
...
mod tests {
    ...
    // Pure `UowStage` tests (wire round-trip, serde, default, ordinal) moved to
    // `camerata_api_types::lifecycle::tests` along with the enum + its pure impl.
