...
- **2026-07-04_deep-discovery-scan-and-reonboarding.md** — a third, opt-in ONBOARDING tier: an open-ended bug + gap DISCOVERY scan (distinct from the mechanical scan and the rule-conformance audit), reasoning over whole subsystems + the repo's stated intent to find latent correctness bugs and functionality gaps. Uses bounded, read-only, subsystem-partitioned reasoning agents (NO recursive spawning), suggests the strongest model, opt-in with a cost estimate, routes findings into the existing triage-to-issues pipeline. Plus RE-ONBOARDING (a point-in-time re-scan of an existing project). Camerata's own "L3 discovers, L2 enforces" thesis turned outward. Motivated by the 2026-07-04 Fable audit (`docs/ARCH_AUDIT_2026-07-04_fable5-*.md`). Naming + 3 scope decisions open.
- **2026-07-02_design-canvas-lifecycle.md** — Design Canvas designs become first-class, lifecycled Units of Work: an explicit `is_design_root: bool` marker (not the shared `draft-<token>` prefix) plus a `design_status` (draft/published/archived) distinct from the dev-run lifecycle; per-project list (`GET /api/projects/:id/designs`), status (`POST /api/designs/:id/status`), and whole-tree delete (`DELETE /api/designs/:id`) endpoints with auto-save and status-badge UI. The planner drafts against the project's configured `HierarchySchema`, and an empty schema resolves to the default ladder via one owner (`resolve_effective`/`effective_hierarchy_schema`) shared by author, materialize, hierarchy GET, and import seeding. Fixes: preserve the AI-drafted child `body` end to end, ground mockup generation on node + parent context with an empty prompt, and repair four client/server contract mismatches. Framed as one adapter over the headless core (adapter ladder staged; GitHub-only publish; artifacts partly staged).
- **2026-07-02_workspace-link-and-sync-panel.md** — Workspace-page additions on the readiness gate: "Link to this folder…" points a repo at an existing local clone via a folder picker, validating the clone's `origin` matches the project identity before recording a machine-local per-repo override (never exported, never clones). Status + readiness now resolve through the override first, counting a folder as cloned only when it is a git checkout whose origin matches `owner/repo`, compared case-insensitively (closing the stuck-paused casing bug); clones outside `<owner>/<repo>` are recognized. Adds a per-repo `<select>` that scopes the page to one repo, branch/commit search (`branch_matches`/`commit_matches`), and matched Pull/Push/Ship button geometry.
- **2026-07-02_rule-repo-scoping.md** — a rule's corpus-authored `scope` dictates its bucket (`bucket_of`): `cross-repo`/`process` are PROJECT-LEVEL (apply project-wide, never emitted into repo files, gates read them from the project store); anything else is REPO-LOCAL (emitted into each chosen repo, scoped by the selection's `repos`). Picking an option ADOPTS a project-level rule but never a repo-local one. Both persist paths unified through one pure `apply_chosen_option` transform (fixing the Applied-table reflection bug), with integration tests. Repo-local rules get a checkbox-per-repo picker IN the rule detail modal (`apply_repo_scope`); project-level rules show "Applies project-wide". The single-repo sentinel (`\u{0}__single_repo__`) is contained at the UI save boundary and the server so it never reaches persisted `repos` or `git clone`. Refines the 2026-06-24 rule selection persistence ADR.
- **2026-07-02_claude-backend-setting.md** — SUPERSEDED (2026-09-22, see `docs/design/2026-09-22_per-project-backend.md`). The CLI/API Claude backend toggle is now a persisted in-app setting (`llm_backend`). A hydration bridge writes it into `CAMERATA_LLM_BACKEND` at startup and on save (precedence: stored setting > env > default `cli`). `anthropic_api_key` joins `ALL_CREDENTIALS` as a keychain-backed credential, hydrated into `ANTHROPIC_API_KEY`. The UI "Claude backend" toggle reveals the Anthropic key row only when API is selected. This toggle is specifically about Claude's two access paths (subscription CLI vs Anthropic API); provider-agnostic model selection is the separate 2026-06-27 runtime. Refines the 2026-06-27 provider-agnostic agent runtime ADR. The keychain-backed-key mechanics still stand; the single global toggle + env precedence do not — backend is per-project now, with one separate global setting for chat only.
- **2026-07-02_bombe-loading-invariant.md** — Invariant: every async spawn that calls an AI endpoint or starts a long-running operation (run, scan, audit, chat turn, prompt authoring, deep-report export) MUST hold a `crate::loading::LoadingGuard` for its duration. The guard is RAII: it increments a global ref-counted `Signal<usize>` on construction and decrements on `Drop`; the Bombe animation runs while the count is positive. A 14-site sweep brought the invariant into full compliance; the 14th missing site (routine-escalation Ask chat) is a direct example of the failure mode the L3 completeness dimension (2026-07-02 ADR) now catches automatically.
