...
**Status: Batch 6 structural (GAP-1, GAP-3, GAP-7) all LANDED on `feat/adapter-ladder-headless-core`, completing the audit batch: `1e02803` (api-types leaf), `9c88bd3` (camerata-llm + Completer->LlmPort rename), `206a067` (GovDevState headless-core state lift, GAP-3), `d699322` (camerata-client), `5a6c3c5` (camerata-mcp, GAP-1 first rung), `113195f` (CLI HTTP adapter, GAP-7), `59a6981` (UI drops the embedded BFF for a spawned camerata-server subprocess, GAP-1 severance). ADR: `docs/decisions/2026-07-08_adapter-ladder-and-headless-core.md`.**
